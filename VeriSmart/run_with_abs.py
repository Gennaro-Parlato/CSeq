...
def run_with_abstraction(category, fa, bitwidth):
    f = fa[0]
    ans = fa[1]
    ...
    if is_data_race_mode:
        prefix += 'dr'
    prefix += '_'
    macro_filepath = base_file_path + '/' + category['relative_path'] + '/' + prefix + f + ".h"
    abs_str = "--abstraction --bit_width "+str(bitwidth) + " --macro-file " + macro_filepath
    ...
    if 'Sequentialization successfully completed' in output.decode():
        ...
        res_out['seq_length'] = length
        copyfile([seq_filepath], [output_file_path + '/' + category['relative_path'] + '/' + f +'/' + prefix + f])
        copyfile([macro_filepath], [output_file_path + '/' + category['relative_path'] + '/' + f +'/' + prefix + f + ".h"])
    
        res_out['seq_time'] = end_time - start_time
        
        print('timeout -k 10 %d ./cbmc-SM %s --unwind %d --no-unwinding-assertions --stop-on-fail' % (
            timeout, seq_filepath, unwind_bound))
        start_time = time()
        p = subprocess.Popen(
        ...
        if 'VERIFICATION FAILED' in output.decode():
            res_out['cbmc_result'] = 'P'
        elif 'VERIFICATION SUCCESSFUL' in output.decode():
            res_out['cbmc_result'] = 'N'
        elif 'core dumped' in output.decode():
            res_out['cbmc_result'] = 'CBMC CRASH ERROR'
        elif 'SYNTAX ERROR' in output.decode():
            res_out['cbmc_result'] = 'CBMC SYNTAX ERROR'
        elif 'CONVERSION ERROR' in output.decode():
            res_out['cbmc_result'] = 'CBMC CONVERSION ERROR'
        else:
            res_out['cbmc_result'] = 'TIMEOUT'

        if res_out['cbmc_result'] in ('P', 'N'):
            try:
                ...
                res_out['variables'] = vars_amount
                res_out['clauses'] = clauses_amount
                res_out['solver_time'] = solver_time
            except:
                pass
        res_out['total_time'] = time_taken + res_out['seq_time']
    else:
        res_out['seq_result'] = 'SEQ ERROR'
    print(res_out)
    return res_out
...
if __name__ == '__main__':
    results = dict()
    reportBlocks = ['WRONG ANSWER - expected N', 'WRONG ANSWER - expected P', 'SEQ ERROR', 'CBMC SYNTAX ERROR', 'CBMC CONVERSION ERROR', 'CBMC CRASH ERROR', 'TIMEOUT', 'ok - got N', 'ok - got P']
    points = 0
    maxpoints = 0
    ...
    for category in categories:
        for fa in category['files']:
            ...
            for bitwidth in range(1,33):
                results[category['relative_path']][f] = run_with_abstraction(category, fa, bitwidth)
                if results[category['relative_path']][f]['seq_result'] == 'SEQ ERROR' or results[category['relative_path']][f]['cbmc_result'] != 'P':
                    break
