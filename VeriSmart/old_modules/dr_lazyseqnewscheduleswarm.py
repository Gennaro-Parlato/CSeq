""" Lazy-CSeq Sequentialization module for data races
    (swarm version of the corresponding name)
"""
VERSION = 'dr_lazyseqnewscheduleswarm-0.1-2019.05.1'
...
class dr_lazyseqnewscheduleswarm(core.module.Translator):
...
# DR data for discarding clearly benign dataraces (i.e., when we have write-write of the same value
    ...
    def loadfromstring(self, string, env):
        ...
        if self.__showThreadLines:   #swarm
            self.showThreadLines()   #swarm
            ...
            #DR end
                ...
                # Case 2: labels
                    ...
                    if not self.__atomic and self.__realStmtCount == -1:   # first statement in a thread
                        self.__stmtCount += 1
                        self.__realStmtCount += 1   #swarm
                        self.__maxInCompound = self.__maxStmtCount
                        threadIndex = self.Parser.threadIndex[self.__currentThread] if self.__currentThread in self.Parser.threadIndex else 0
                        stamp = '__CSEQ_rawline("IF(%s,%s,%s%s_%s)");\n' % (threadIndex,str(self.__realStmtCount), self.__startChar, self.__currentThread, str(self.__realStmtCount+1))        #swarm
                        code,dr_part1,dr_part2 = self.dr_codeParts(stmt.stmt,threadIndex,str(self.__realStmtCount))  #DR
                ...
                # Case 3: all the rest....
                    ...
                    if not self.__atomic and self.__realStmtCount == -1:   # first statement in a thread
                        ...
                        threadIndex = self.Parser.threadIndex[self.__currentThread] if self.__currentThread in self.Parser.threadIndex else 0
                        stamp = '__CSEQ_rawline("IF(%s,%s,%s%s_%s)");\n' % (threadIndex,str(self.__realStmtCount), self.__startChar, self.__currentThread, str(self.__realStmtCount+1))
                        code,dr_part1,dr_part2 = self.dr_codeParts(stmt,threadIndex,str(self.__realStmtCount))  #DR
                        newStmt = dr_part1 + stamp + dr_part2 + code +';\n'
                    elif (not self.__visit_funcReference and (
                        (type(stmt) == pycparser.c_ast.FuncCall and stmt.name.name == '__CSEQ_atomic_begin') or
                        (not self.__atomic and
...
#            main +="__CSEQ_rawline(\"__cs_tsplusone=%s;\");\n" % ( (round+1) * ( self.__threadbound+1) )  #POR
            ...
            if ts <= maxts :   #DR
                main +="             if(__cs_dr_ts == %s) __cs_dataraceDetectionStarted=1;\n" % ts  #DR
            main +="             __cs_main_thread();\n"
            main +="             if(__cs_dataraceSecondThread & (__cs_tmp_t0_r%s > __cs_pc[0])) __cs_dataraceContinue=0;\n" % (round) #DR
            if ts <= maxts :   #DR 
                main +="             if(__cs_dataraceDetectionStarted) __cs_dataraceSecondThread=1;\n"  #DR
#            main +="             __CSEQ_assume(__cs_is_por_exec);\n" #POR
            main +="             __cs_pc[0] = __cs_pc_cs[0];\n"
...
#                    main +="             __CSEQ_assume(__cs_is_por_exec);\n" #POR
        ...
        #main +="           if (__cs_dr_ts > %s & __cs_dataraceContinue & __cs_active_thread[0]) {\n" % ((round-1) * (self.__threadbound+1)) #DR
        #if self.__guess_cs_only:
        #    main +="             __cs_pc_cs[0] = __cs_tmp_t0_r%s;\n" % (ROUNDS)
        #else:
        #    main +="             __cs_pc_cs[0] = __cs_pc[0] + __cs_tmp_t0_r%s;\n" % (ROUNDS)
...
#DR overriding built on the POR version
    def visit_Assignment(self, n):
        ...
        if self.__laddress != '' and self.__laddress not in self.__sharedVarsW:
    ...
    def visit_UnaryOp(self, n):
        ...
        if n.op == '&':         #POR
           ...
           if self.__visitingLPart == True:
               ...
               if operand not in self.__sharedVarsR: 
