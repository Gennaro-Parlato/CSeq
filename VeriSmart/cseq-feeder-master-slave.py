    ...
    if isSlave:
        ...
        if "--dr" in cseqenv.cmdline:
            cseqenv.chainfile = "modules/%s.chain" % core.utils.extractparamvalue(cseqenv.cmdline, "-C", "--chain",
                                                                                  core.config.defaultDRchainSlave)
        elif "--mydr" in cseqenv.cmdline:
            cseqenv.chainfile = "modules/%s.chain" % core.utils.extractparamvalue(cseqenv.cmdline, "-C", "--chain",
                                                                              "dr_assertionChecking_slave")
        else:
            cseqenv.chainfile = "modules/%s.chain" % core.utils.extractparamvalue(cseqenv.cmdline, "-C", "--chain",
                                                                                  core.config.defaultchainSlave)
    elif isMaster:
        # Extract the configuration from the command-line or set it to the default.
        if "--dr" in cseqenv.cmdline:
            cseqenv.chainfile = "modules/%s.chain" % core.utils.extractparamvalue(cseqenv.cmdline, "-C", "--chain",
                                                                                  core.config.defaultDRchainMaster)
                                                                                  
        elif "--mydr" in cseqenv.cmdline:
            cseqenv.chainfile = "modules/%s.chain" % core.utils.extractparamvalue(cseqenv.cmdline, "-C", "--chain",
                                                                              "dr_assertionChecking_master")
        else:
            cseqenv.chainfile = "modules/%s.chain" % core.utils.extractparamvalue(cseqenv.cmdline, "-C", "--chain",
                                                                                  core.config.defaultchainMaster)
...
def geometricSeriesLimit(numberChildProcess, n):
    return math.ceil((1 - pow(numberChildProcess, n)) / (1 - numberChildProcess)) if numberChildProcess>1 else n

def smartBroadcast(masterFilename):

    height = math.log(numberProcess, numberChildProcess)
    heightRounded = math.ceil(height)
    numberInternalNodes = geometricSeriesLimit(numberChildProcess, heightRounded - 1) - math.floor(
        ((geometricSeriesLimit(numberChildProcess, heightRounded) - numberProcess) / numberChildProcess))

    if isMaster:

        file = open(str(masterFilename))
        sourceFileString = file.read()
        file.close()

        if 0 <= rank <= numberInternalNodes:
            for processID in range(rank * numberChildProcess + 1,
                                   rank * numberChildProcess + numberChildProcess + 1):
                if processID <= numberProcess - 1:
                    comm.send(sourceFileString, dest=processID)

        cseqenv.inputfile = masterFilename
        #print(masterFilename)

    if isSlave:

        sourceFileString = comm.recv(source=MPI.ANY_SOURCE)
        slaveFilename = str("").join(str(masterFilename).split(".")[:-1]) + "_" + str(rank) + ".c"
        open(slaveFilename, 'w').close()
        file = open(slaveFilename, "a+")
        file.write(sourceFileString)
        file.close()

        cseqenv.inputfile = slaveFilename
        #print(slaveFilename)
