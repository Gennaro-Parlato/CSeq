...
"""
VERSION = 'lazyseqai-0.1-2016.08.10'
...
class lazyseqai(core.module.Translator):
    ...
    def visit_Compound(self, n):
        ...
        if n.block_items:
            for stmt in n.block_items:
                ...
                ##if type(stmt) == pycparser.c_ast.FuncCall and self._parenthesize_unless_simple(stmt.name) == core.common.changeID['pthread_exit']:
                if type(stmt) == pycparser.c_ast.FuncCall and stmt.name.name == core.common.changeID['pthread_exit']:
                    self.__stmtCount += 1
                    ...
                    if self.Parser.funcBlockOut[self.__currentThread] != 'void':
                        returnStr = 'STOP_NONVOID(%s)' % (self.__stmtCount)
                    else:
                        returnStr = 'STOP_VOID(%s)' % (self.__stmtCount)
                ...
                # Case 2: labels
                    ...
                    if not self.__atomic and self.__stmtCount == -1:   # first statement in a thread
                        ...
                        stamp = '__CSEQ_rawline("%s%s_%s: IF(%s,%s,%s%s_%s)");\n' % (self.__startChar, self.__currentThread, self.__stmtCount,threadIndex,str(self.__stmtCount), self.__startChar, self.__currentThread, str(self.__stmtCount+1))
