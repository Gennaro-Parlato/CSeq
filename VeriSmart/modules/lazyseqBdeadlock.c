...
void *__cz_threadargs[THREADS+1];
void *__cs_thread_joinargs[THREADS+1];

int __cs_create(__cs_t *__cs_id, void *__cs_attr, void *(*__cs_thread_function)(void*), void *__cz_arg, int __cs_threadID) {
...
int __cs_mutex_lock(__cs_mutex_t *__cs_mutex_to_lock) {
	...
	if (*__cs_mutex_to_lock==-1) {
		*__cs_mutex_to_lock = __cs_thread_index+1;
		if (__cs_chaincounter==0 & __CSEQ_nondet_int()) {
		...
		if (__cs_chaincounter > 0 & __cs_chain[__cs_chaincounter-1]==*__cs_mutex_to_lock & __CSEQ_nondet_int()) {
